...
export default function Page() {
  ...
  const [data, setData] = useState<Record<string, { imgURL?: string; name?: string }>>({});
  ...
  useEffect(() => {
    if (!session?.user?.oauthId) return;
    axios.get(`/api/getUserByOauthId?oauthid=${session.user.oauthId}`)
  ...
  useEffect(() => {
    ...
    axios.get(`/api/getConversationMessages?convoID=${activeConversation}`)
  ...
  useEffect(() => {
    messages.forEach((msg) => {
      if (msg.senderId !== user?._id && !companies[msg.senderId]) {
        axios.get(`/api/getCompanyByID?companyID=${msg.senderId}`)
          .then((res) => {
        ...
        <div className="my-8 ml-8 w-[25%] h-full border border-gray-300 rounded-lg p-4">
          ...
          <div className="flex justify-center mt-3">
            <label className="input">
              <FaSearch />
              <input type="search" className="grow" placeholder="Kërko një kontakt.." />
            </label>
          </div>
            ...
            {messages.map((message) => {
              ...
              return (
                <div key={message._id} className={`chat ${isCurrent ? "chat-end" : "chat-start"}`}>
                  ...
                  <div className={`chat-bubble ${isCurrent ? "bg-blue-500 text-white" : "bg-gray-200 text-black"}`}>
                    {message.content}
                  </div>
                </div>
              );
          ...
          <div className="flex items-center h-16 mt-auto border border-gray-300 rounded-full shadow-lg px-4">
            ...
            <button
              ...
              onClick={() => {
                if (!activeConversation) return;
                axios.get(
                  `/api/sendMessage?oid=${session?.user.oauthId}&message=${enteredMessage}&channel=${activeConversation}`
                );
                setEnteredMessage("");
              }}
            >
